...
# ═══════════════════════════════════════════════════════
#  CONTACT REQUEST  (when someone taps Contact on a swap)
#  Mini app / inline Contact button writes to Firebase
#  Bot job picks it up and sends confirm buttons to both
# ═══════════════════════════════════════════════════════
async def handle_contact_request(context: ContextTypes.DEFAULT_TYPE):
    """Job: runs every 60s, checks Firebase for new contact requests."""
    if not FIREBASE_ON: return
    try:
        requests = fb_get("contact_requests") or {}
        for key, data in requests.items():
            if data.get("notified"): continue
            a_uid  = data.get("swap_owner_id")
            b_uid  = data.get("contactor_id")
            sid    = data.get("swap_id","")
            a_name = data.get("swap_owner_name","Someone")
            b_name = data.get("contactor_name","Someone")
            a_lang = ulang(a_uid) if a_uid else "en"
            b_lang = ulang(b_uid) if b_uid else "en"

            # Notify A that B wants to swap
            try:
                await context.bot.send_message(
                    chat_id=a_uid, parse_mode="Markdown",
                    text="👋 *" + str(b_name) + "* wants to swap seats with you!\n\n_Meet in the train, exchange berths, then both tap Confirm below to earn +10 pts!_",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("✅ Confirm Swap Done", callback_data=f"confirm_{sid}")],
                    ])
                )
            except Exception as e:
                print(f"Contact notify A error: {e}")

            # Notify B with confirm button too
            try:
                b_sid = f"cr_{key}"   # synthetic swap id for B's confirm
                fb_set(f"contact_confirms/{b_sid}", {"a_uid": a_uid, "b_uid": b_uid, "real_sid": sid})
                await context.bot.send_message(
                    chat_id=b_uid, parse_mode="Markdown",
                    text="📩 *You contacted " + str(a_name) + " for a seat swap.*\n\n_After the swap, both of you tap Confirm to earn +10 pts!_",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("✅ Confirm Swap Done", callback_data=f"confirm_cr_{key}")],
                    ])
                )
            except Exception as e:
                print(f"Contact notify B error: {e}")

            # Mark notified
            fb_upd(f"contact_requests/{key}", {"notified": True})
    except Exception as e:
        print(f"Contact request job error: {e}")
...
async def find_got_train(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["flow"] = ""
    swaps = all_swaps()
    results = {k:v for k,v in swaps.items()
               if v.get("train")==raw and v.get("status")=="active"
               and v.get("user_id") != uid}     # ← never show own request
    tname = TRAINS.get(raw,"")
    if not results:
        await update.message.reply_text(
...
async def confirm_swap(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    uid  = update.effective_user.id; lang = ulang(uid)
    raw  = update.callback_query.data.replace("confirm_","")

    # ── Contact-request confirm (cr_ prefix) ──────────────────────────
    if raw.startswith("cr_"):
        cr_key  = raw[3:]                # strip "cr_"
        cr_data = fb_get(f"contact_confirms/{raw}") or {}
        a_uid   = cr_data.get("a_uid")
        b_uid   = cr_data.get("b_uid")
        real_sid= cr_data.get("real_sid", raw)
        sid = real_sid
    else:
        sid   = raw
        a_uid = None; b_uid = None

    key = f"confirmed/{uid}_{sid}"
    if fb_get(key):
        await update.callback_query.edit_message_text(
            "✅ Already confirmed! Waiting for the other person.",
            reply_markup=kb_home()); return
    ...
    # For contact-request swaps, also check the other sid variant
    other_uid = None
    if a_uid and b_uid:
        other_uid = b_uid if uid == a_uid else a_uid
        # Check if other party already confirmed (either variant)
        other_confirmed = any(
            f"confirmed/{other_uid}_{sid}" == f"confirmed/{p.split('_')[0]}_{sid}"
            for p in parties
        ) or fb_get(f"confirmed/{other_uid}_{sid}") or fb_get(f"confirmed/{other_uid}_cr_{raw[3:] if raw.startswith('cr_') else ''}")

        if other_confirmed or len(parties) >= 2:
            sd, pts   = award(uid)
            o_sd, o_pts = award(other_uid)
            fb_set(f"matches/{sid}", {"completed": datetime.now().strftime("%d %b %Y %I:%M %p")})
            try:
                await ctx.bot.send_message(
                    chat_id=other_uid, parse_mode="Markdown",
                    text=s("confirmed_both", ulang(other_uid), pts=o_pts, badge=get_badge(swaps=o_sd), bar=progress_bar(o_sd))
                )
            except: pass
            await update.callback_query.edit_message_text(
    ...
    # ── Auto-match confirm (original flow) ───────────────────────────
    if len(parties) >= 2:
        sd, pts = award(uid)
        for ck in parties:
            try:
...
# Job: check contact requests every 60 seconds
app.job_queue.run_repeating(handle_contact_request, interval=60, first=10)
