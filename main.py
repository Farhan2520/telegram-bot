...
MINI_APP_URL = os.getenv("MINI_APP_URL", "")  # Set this in Railway env vars
...
STRINGS = {
    ...
    "match_found": {
        ...
        "ur": "🎉 *فوری میچ ملا!*\n\n👤 *{name}* {badge}\n🚉 بوگی: *{coach}*\n💺 ہے: سیٹ *{cseat}* (*{cberth}*)\n🔄 چاہیے: سیٹ *{wseat}* (*{wberth}*)\n\n📩 رابطہ: {contact}\n\n_تبادلے کے بعد ✅ تصدیق کریں → +10 pts!_",
    },
    "confirmed_1": {
    ...
    "search_enter": {
        "en": "🔍 *Find Seat*\n\nEnter *train number* to search available seat swaps:",
    ...
    "help_text": {
        "en": (
            "❓ *How to use SeatSwap*\n\n"
            "1️⃣ Tap *🔄 Change Seat*\n"
            "   → Train → Coach → Your seat no → Wanted seat no → PNR (optional)\n\n"
            "2️⃣ Tap *🔍 Find Seat*\n"
        ...
        "ur": (
            ...
            "4️⃣ تبادلے کے بعد *✅ تصدیق کریں* → دونوں کو +10 pts!\n\n"
            "🏅 *بیج:* 🌱 نیا • 🎫 باقاعدہ (3+)\n"
...
def get_user(uid):
    return fb_get(f"users/{uid}") or {
        ...
        "swaps_done": 0, "badge": "🌱 Newcomer", "banned": False,
        "reports": 0, "special_user": False, "special_badge": "",
    }
...
def save_swap(user, d):
    ...
    sw = {
        ...
        "train":      d["train"],
        "train_name": d.get("train_name", d["train"]),
        "coach_type": d["ct"],
        "coach":      d["coach"],
        "cur_seat":   d["cseat"],
        "cur_berth":  d["cberth"],
        "want_seat":  d.get("wseat", "Any"),
        "want_berth": d.get("wberth", "Any"),
        "pnr":        d.get("pnr", ""),
        "status":     "active",
...
def find_match(sw):
    ...
    nw  = sw["want_berth"].lower()
    nc  = sw["cur_berth"].lower()
    ...
    for k, s in all_swaps().items():
        ...
        tw  = s.get("want_berth", "").lower()
        tc  = s.get("cur_berth",  "").lower()
        ...
        if not berth_ok:
            continue
        score = 1
        if nws and nws != "Any" and nws == tcs: score += 10
        if tws and tws != "Any" and tws == ncs: score += 10
        if score > best_score:
            best_score = score
            best = s
    return best
...
def kb_ct():
    ...
    for i in range(0, len(items), 2):
        rows.append([
            InlineKeyboardButton(v["label"], callback_data=f"ct_{k}")
...
def kb_any_seat(lang):
    labels = {
        "en": "✅ Any Seat", "hi": "✅ कोई भी सीट",
        "bn": "✅ যেকোনো সিট", "ur": "✅ کوئی بھی سیٹ",
    }
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(labels.get(lang, "✅ Any Seat"), callback_data="wseat_any")],
        [InlineKeyboardButton("❌ Cancel", callback_data="home")],
    ])

def kb_yesno(yes_cb, no_cb, lang):
    yes_lbl = {"en": "✅ Yes", "hi": "✅ हाँ", "bn": "✅ হ্যাঁ", "ur": "✅ ہاں"}.get(lang, "✅ Yes")
    no_lbl  = {"en": "❌ Re-enter", "hi": "❌ दोबारा", "bn": "❌ আবার", "ur": "❌ دوبارہ"}.get(lang, "❌ Re-enter")
    return InlineKeyboardMarkup([[
        InlineKeyboardButton(yes_lbl, callback_data=yes_cb),
        InlineKeyboardButton(no_lbl,  callback_data=no_cb),
    ]])

def kb_skip(lang):
    lbl = {"en": "⏭️ Skip", "hi": "⏭️ छोड़ें", "bn": "⏭️ এড়িয়ে", "ur": "⏭️ چھوڑیں"}.get(lang, "⏭️ Skip")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(lbl, callback_data="pnr_skip")],
        [InlineKeyboardButton("❌ Cancel", callback_data="home")],
    ])
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_banned(user.id):
        await rply(update, "🚫 Your account has been suspended.")
        return
    lang   = ulang(user.id)
...
async def cs_got_train(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not re.match(r"^\d{4,6}$", raw):
        await update.message.reply_text(
            "❌ Train number should be 4–6 digits. Try again:", parse_mode="Markdown"
        )
        return
    ...
    if tname:
        await update.message.reply_text(
            s("step_coach_type", lang, num=raw, name=tname),
...
async def cs_got_ct(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text(
        ...
        reply_markup=kb_coaches(COACH_TYPES[code]["coaches"]),
        parse_mode="Markdown",
    )
...
async def cs_got_cseat(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not raw.isdigit() or not (1 <= int(raw) <= mx):
        await update.message.reply_text(s("invalid_seat", lang, ct=ct, max=mx), parse_mode="Markdown")
        return
...
async def cs_got_wseat_num(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not raw.isdigit() or not (1 <= int(raw) <= mx):
        await update.message.reply_text(
            s("invalid_seat", lang, ct=ct, max=mx),
            reply_markup=kb_any_seat(lang),
            parse_mode="Markdown",
        )
        return
...
async def cs_got_wberth(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lang = ulang(update.effective_user.id)
    await update.callback_query.edit_message_text(
        s("step_pnr", lang), reply_markup=kb_skip(lang), parse_mode="Markdown"
    )
...
async def cs_got_pnr(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not re.match(r"^\d{10}$", raw):
        await update.message.reply_text(
            s("invalid_pnr", lang), reply_markup=kb_skip(lang), parse_mode="Markdown"
        )
        return
...
async def cs_finish(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        try:
            ul2 = ulang(match["user_id"])
            ume = get_user(user.id)
...
async def find_got_train(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    rows  = [
        [InlineKeyboardButton(f"🚉 {c}  —  {len(vs)} request(s)", callback_data=f"vc_{raw}_{c}")]
...
async def show_coach(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for i, req in enumerate(results, 1):
        ...
        ws_l = f" (prefers seat *{req['want_seat']}*)" if req.get("want_seat") and req["want_seat"] != "Any" else ""
        text += (
            f"*#{i}  {req['name']}*  {req.get('badge', '🌱 Newcomer')}\n"
            f"💺 Seat *{req.get('cur_seat','?')}* ({req.get('cur_berth','?')})  →  🔄 *{req.get('want_berth','Any')}*{ws_l}{pnr}\n"
...
async def confirm_swap(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if fb_get(key):
        await update.callback_query.edit_message_text("✅ Already confirmed!", reply_markup=kb_home())
        return
    fb_set(key, True)
                ...
                if o_uid != uid:
                    ...
                    try:
                        await ctx.bot.send_message(
                            ...
                            text=s("confirmed_both", ulang(o_uid),
                                   pts=o_pts, badge=get_badge(swaps=o_sd), bar=progress_bar(o_sd)),
                        )
                    except:
                        pass
            except:
...
async def report_user(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if reporter == reported:
        await update.callback_query.answer("❌ Can't report yourself!", show_alert=True)
        return
    key = f"reports/{reporter}_{reported}"
    if fb_get(key):
        await update.callback_query.answer("Already reported.", show_alert=True)
        return
    rid = str(uuid.uuid4())[:8]
    fb_set(key, True)
    fb_set(f"all_reports/{rid}", {
        "reporter":  reporter,
        "reported":  reported,
        "ts":        datetime.now().strftime("%d %b %Y %I:%M %p"),
    })
    ...
    for aid in ADMIN_IDS:
        try:
            ...
            await ctx.bot.send_message(
                ...
                text=(
                    f"🚩 *New Report*\n\n"
                    f"Reporter: {ru.get('name','')} (ID:`{reporter}`)\n"
                    f"Reported: {u.get('name','')} (ID:`{reported}`)\n"
                    f"Total reports on reported: *{count}*"
                ),
            )
        except:
            pass
    if count >= 3:
        ...
        del_my_swap(reported)
        try:
            await ctx.bot.send_message(
                chat_id=reported, text="🚫 Your SeatSwap account has been suspended."
            )
        except:
            pass
...
async def profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    pts   = u.get("points", 0)
    badge = get_badge(user.id)
    special_line = "\n🌟 *Official SeatSwap Family*" if u.get("special_user") else ""
    sp_badge = u.get("special_badge", "")
    sp_line  = f"\n🏅 Special: *{sp_badge}*" if sp_badge else ""
    await rply(update,
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"👤 *Profile*{special_line}\n"
        f"━━━━━━━━━━━━━━━━━━━\n\n"
        f"Name:     *{user.first_name}*\n"
        f"Username: @{user.username or 'not set'}\n"
        f"Badge:    {badge}{sp_line}\n\n"
        f"✅ Swaps Done: *{sd}*\n"
        f"⭐ Points:     *{pts}*\n\n"
        f"📈 *Progress:*\n{progress_bar(sd)}",
        kb_home(),
...
async def feedback_got_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for aid in ADMIN_IDS:
        try:
            ...
            pass
    await update.message.reply_text(
        s("feedback_done", lang), reply_markup=kb_home(), parse_mode="Markdown"
    )
...
# ═══════════════════════════════════════════════════════
#  ADMIN PANEL COMMAND (mini inline panel)
# ═══════════════════════════════════════════════════════
async def admin_panel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    fb_icon  = "🟢 Online" if FIREBASE_ON else "🔴 Offline"
    text = (
        f"🛠 *Admin Panel*\n"
        f"━━━━━━━━━━━━━━━━━━━\n\n"
        f"🗄 Firebase: *{fb_icon}*\n"
        ...
        f"🚫 Banned:   *{banned}*\n\n"
        f"📌 *Quick Commands:*\n"
        f"`/stats` `/allusers` `/activeusers`\n"
        f"`/totalswaps` `/feedbacks` `/checkdb`\n"
        f"`/ban <id>` `/unban <id>`\n"
        f"`/userinfo <id>` `/broadcast <msg>`\n"
        f"`/givebadge <id> <badge>` `/removebadge <id>`\n"
        f"`/viewbadge <id>` `/setfamily <id>`\n"
        f"`/deleteswap <id>`"
    )
...
async def admin_allusers(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, u in list(users.items())[:20]:
        ...
        text  += f"{status} {u.get('name','')} (@{u.get('username','?')}) — {u.get('badge','')} — {u.get('swaps_done',0)} swaps\n"
    if len(users) > 20:
...
async def admin_activeusers(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in uids[:20]:
        ...
        sw = my_swap(uid)
        if sw:
            text += f"• {u.get('name','')} — Train {sw.get('train','')} Coach {sw.get('coach','')}\n"
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def admin_givebadge(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(ctx.args) < 2:
        ...
        await update.message.reply_text(
            f"Usage: /givebadge <user_id> <badge>\nOptions: {opts}\nExample: /givebadge 123456 Master"
        )
        return
    uid       = int(ctx.args[0])
    badge_key = ctx.args[1].capitalize()
    if badge_key not in SPECIAL_BADGES:
        await update.message.reply_text(f"❌ Unknown badge. Options: {', '.join(SPECIAL_BADGES.keys())}")
        return
    badge_val = SPECIAL_BADGES[badge_key]
    fb_upd(f"users/{uid}", {"special_badge": badge_val, "badge": badge_val})
    u = get_user(uid)
    await update.message.reply_text(
        f"✅ {badge_val} badge given to *{u.get('name', uid)}*.", parse_mode="Markdown"
    )
    try:
        await ctx.bot.send_message(
            chat_id=uid, parse_mode="Markdown",
            text=f"🎉 You've been given a special badge: *{badge_val}*\n\nKeep swapping! 🚆",
        )
    except:
        pass
...
async def admin_removebadge(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    if not ctx.args:
        await update.message.reply_text("Usage: /removebadge <user_id>")
        return
    uid = int(ctx.args[0])
    u   = get_user(uid)
    sd  = u.get("swaps_done", 0)
    ...
    await update.message.reply_text(
        f"✅ Special badge removed from *{u.get('name', uid)}*.", parse_mode="Markdown"
    )
...
async def admin_viewbadge(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    if not ctx.args:
        await update.message.reply_text("Usage: /viewbadge <user_id>")
        return
    uid = int(ctx.args[0])
    u   = get_user(uid)
    await update.message.reply_text(
...
async def admin_setfamily(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    if not ctx.args:
        await update.message.reply_text("Usage: /setfamily <user_id>")
        return
    uid = int(ctx.args[0])
    fb_upd(f"users/{uid}", {"special_user": True, "badge": "🌟 Official SeatSwap Family"})
    u = get_user(uid)
    await update.message.reply_text(
        f"✅ *{u.get('name', uid)}* is now Official SeatSwap Family 🌟", parse_mode="Markdown"
    )
    try:
        await ctx.bot.send_message(
            chat_id=uid, parse_mode="Markdown",
            text="🌟 You are now an *Official SeatSwap Family* member!\n\nSpecial badge added to your profile.",
        )
    except:
        pass
...
async def admin_ban(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.send_message(chat_id=uid, text="🚫 Your SeatSwap account has been suspended.")
    except:
        pass
...
async def admin_unban(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.send_message(chat_id=uid, text="✅ Your SeatSwap account has been restored.")
    except:
        pass
...
async def admin_userinfo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    sw       = my_swap(uid)
    swap_inf = (
        f"🔄 Active: Train {sw['train']} Coach {sw['coach']} Seat {sw.get('cur_seat','?')}"
        if sw else "No active swap"
    )
    await update.message.reply_text(
        f"👤 *User Info*\n\n"
        f"ID:       `{uid}`\n"
        ...
        f"Badge:    {u.get('badge','')}\n"
        f"Special:  {u.get('special_badge') or 'None'}\n"
...
async def admin_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in users:
        try:
            await ctx.bot.send_message(
                chat_id=int(uid), parse_mode="Markdown",
...
async def admin_deleteswap(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    if not ctx.args:
        await update.message.reply_text("Usage: /deleteswap <user_id>")
        return
    uid     = int(ctx.args[0])
    deleted = del_my_swap(uid)
    await update.message.reply_text(
        f"{'✅ Deleted' if deleted else '❌ No active swap'} for user `{uid}`.",
        parse_mode="Markdown",
    )
...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Menu labels — all languages
    if any(k in text for k in ["Change Seat", "सीट बदलें", "সিট বদলান", "سیٹ بدلیں"]):
        await cs_start(update, ctx); return
    if any(k in text for k in ["Find Seat", "सीट खोजें", "সিট খুঁজুন", "سیٹ تلاش"]):
        await find_start(update, ctx); return
    if any(k in text for k in ["My Request", "मेरा अनुरोध", "আমার অনুরোধ", "میری درخواست"]):
        await my_request(update, ctx); return
    if any(k in text for k in ["Profile", "प्रोफ़ाइल", "প্রোফাইল", "پروفائل"]):
        await profile(update, ctx); return
    if any(k in text for k in ["Language", "भाषा", "ভাষা", "زبان"]):
        await choose_lang(update, ctx); return
    if any(k in text for k in ["Help", "मदद", "সাহায্য", "مدد"]):
        await help_cmd(update, ctx); return
    if any(k in text for k in ["Feedback", "फ़ीडबैक", "ফিডব্যাক", "فیڈبیک"]):
        await feedback_start(update, ctx); return
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Train confirm / retry
        ...
        await update.callback_query.edit_message_text(
            s("step_train", lang), reply_markup=kb_cancel(), parse_mode="Markdown"
        )
    ...
    # Current seat confirm / retry
    elif data == "cs_cseat_ok":
        ctx.user_data["flow"] = "cs_wseat_num"
        await update.callback_query.edit_message_text(
            s("step_want_seat", lang), reply_markup=kb_any_seat(lang), parse_mode="Markdown"
        )
    ...
    # Wanted berth / back
    elif data.startswith("wb_") and data != "wb_back": await cs_got_wberth(update, ctx)
    elif data == "wb_back":
        ctx.user_data["flow"] = "cs_wseat_num"
        await update.callback_query.edit_message_text(
            s("step_want_seat", lang), reply_markup=kb_any_seat(lang), parse_mode="Markdown"
    ...
    # Confirm / report / language
        ...
        try:
            await update.callback_query.edit_message_text("⚠️ Error. Returning...", reply_markup=kb_home())
        except:
            pass
    ...
    try:
        if isinstance(update, Update):
            ...
            if update.callback_query:
                try:
                    await update.callback_query.edit_message_text(
                        "⚠️ Something went wrong. Returning to menu.", reply_markup=kb_home()
                    )
                except:
                    pass
            elif update.message:
                await update.message.reply_text(
                    "⚠️ Something went wrong. Use the menu below:", reply_markup=kb_menu(lang)
                )
    except:
        pass
...
application.add_handler(CommandHandler("myrequest",  my_request))
application.add_handler(CommandHandler("profile",    profile))
application.add_handler(CommandHandler("language",   choose_lang))
application.add_handler(CommandHandler("feedback",   feedback_start))
application.add_handler(CommandHandler("myid",       my_id))
application.add_handler(CommandHandler("cancel",     cancel_req))
...
application.add_handler(CommandHandler("activeusers",  admin_activeusers))
application.add_handler(CommandHandler("totalswaps",   admin_totalswaps))
application.add_handler(CommandHandler("feedbacks",    admin_feedbacks))
application.add_handler(CommandHandler("checkdb",      admin_checkdb))
application.add_handler(CommandHandler("givebadge",    admin_givebadge))
application.add_handler(CommandHandler("removebadge",  admin_removebadge))
application.add_handler(CommandHandler("viewbadge",    admin_viewbadge))
application.add_handler(CommandHandler("setfamily",    admin_setfamily))
application.add_handler(CommandHandler("ban",          admin_ban))
application.add_handler(CommandHandler("unban",        admin_unban))
application.add_handler(CommandHandler("userinfo",     admin_userinfo))
application.add_handler(CommandHandler("broadcast",    admin_broadcast))
application.add_handler(CommandHandler("deleteswap",   admin_deleteswap))
